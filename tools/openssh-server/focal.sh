...
IMAGE=registry.gitlab.com/opcal/ubuntu:focal
...
docker image tag openssh-server:${TAG_VERSION} ${CI_REGISTRY}/opcal/openssh-server:focal-${TIMESTAMP}
docker image tag openssh-server:${TAG_VERSION} ${CI_REGISTRY}/opcal/openssh-server:focal
docker push ${CI_REGISTRY}/opcal/openssh-server:focal-${TIMESTAMP}
