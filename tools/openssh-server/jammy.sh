...
IMAGE=registry.gitlab.com/opcal/ubuntu:jammy
...
docker image tag openssh-server:${TAG_VERSION} ${CI_REGISTRY}/opcal/openssh-server:jammy-${TIMESTAMP}
docker image tag openssh-server:${TAG_VERSION} ${CI_REGISTRY}/opcal/openssh-server:jammy
docker push ${CI_REGISTRY}/opcal/openssh-server:jammy-${TIMESTAMP}
